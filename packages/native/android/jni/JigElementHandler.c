...
static cJSON *build_visible_summary(cJSON *elements) {
    ...
    cJSON_ArrayForEach(el, elements) {
        ...
        if (comp && cJSON_IsString(comp)) cJSON_AddStringToObject(entry, "component", comp->valuestring);
...
jig_handler *jig_android_find_element_handler_create(jig_jsbridge *bridge) {
...
jig_handler *jig_android_find_elements_handler_create(jig_jsbridge *bridge) {
    jig_handler *h = calloc(1, sizeof(jig_handler));
