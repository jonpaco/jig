...
static cJSON *screenshot_handle(jig_handler *self, cJSON *params,
    ...
    /* Capture via PixelCopy — reads from the HW-accelerated surface compositor.
     * Retry up to 3 times — the surface may not be ready yet on headless
     * emulators (error code 3 = ERROR_SOURCE_NO_DATA). */
    jint copy_result = -1;
    ...
    if (copy_result != 0) {
        ...
        if (attached) (*jvm)->DetachCurrentThread(jvm);
        char msg[64];
        snprintf(msg, sizeof(msg), "PixelCopy failed with code %d after retries", (int)copy_result);
