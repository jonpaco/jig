...
extern jig_handler *jig_find_element_handler_create(void);
extern jig_handler *jig_find_elements_handler_create(void);
...
jig_server *jig_bootstrap_server(const char *name, const char *bundle_id,
    ...
    // 3. Create handlers — must be static since dispatcher stores pointers
    static jig_handler *handlers[4];
    handlers[0] = jig_handshake_create();
    handlers[1] = jig_screenshot_handler_create();
    handlers[2] = jig_find_element_handler_create();
    handlers[3] = jig_find_elements_handler_create();
